...
export default function SearchClient({ query, initialResults }) {
  ...
  const isDataEmpty = initialResults.berita.length === 0 && initialResults.pengaduan.length === 0;
      ...
      <div className="relative py-16 px-8 flex items-center justify-center overflow-hidden">
        ...
        <div className="relative z-10 text-center">
          ...
          <p className="text-emerald-300 text-sm uppercase tracking-[0.2em] font-bold">
            "{query}" • {(initialResults.berita.length + initialResults.pengaduan.length)} Hasil Ditemukan
          </p>
        </div>
      </div>
      ...
      <div className="w-full px-6 md:px-12 py-12">
        {isDataEmpty ? (
          <div className="text-center py-20 bg-white rounded-3xl shadow-sm border border-slate-100 max-w-2xl mx-auto">
            ...
            <p className="text-slate-500 mt-2">Coba gunakan kata kunci lain untuk "{query}".</p>
          </div>
        ) : (
          <div className="space-y-16 max-w-7xl mx-auto">
            {/* SECTION BERITA */}
            {initialResults.berita.length > 0 && (
              <section>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                  {initialResults.berita.map((item) => (
...
// --- KOMPONEN KECIL (Definisikan di dalam file yang sama agar tidak error) ---
  ...
  return (
    <div onClick={onOpen} className="group bg-white rounded-2xl shadow-sm hover:shadow-xl border border-slate-100 overflow-hidden flex flex-col transition-all duration-300 cursor-pointer h-full">
      <div className="relative h-52 overflow-hidden bg-slate-100">
        <img src={item.foto_bukti_url || "/images/placeholder-report.jpg"} className="w-full h-full object-cover transition duration-700 group-hover:scale-105" alt="Bukti" />
        <div className="absolute top-4 left-4 bg-slate-800/80 backdrop-blur-sm text-white text-[10px] font-bold px-3 py-1 rounded-full uppercase tracking-wider shadow-lg">{item.kategori || 'Umum'}</div>
      </div>
      <div className="p-6 flex flex-col flex-1">
